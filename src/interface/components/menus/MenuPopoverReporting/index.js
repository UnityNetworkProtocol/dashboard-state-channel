...
import { financialFolder } from 'assets/svg'
import {
 Flex, Box,
...
export default props => <Flex diretion={['column', 'row']} >
  <Box px={[10]} {...props.styled} >
    <PopoverPure delta='MenuReporting' body={Body} >
      <Flex align='center' direction='column' justify="center" >
        <SVG w={30} h={30} svg={financialFolder}/>
        <Heading f={[1]} fw='300' >
          Reporting
        </Heading>
      </Flex>
...
const Body = props => <Box w={[1,1,675]} boxShadow={2} hover={{boxShadow: 3}}  br={10} >
  <Flex direction={['column', 'row']} >
    <Box w={[1,1,0.45]} gradient='bluePurple' color='white' p={[10,20]} bs={1} >
      <PerfectScrollbar>
        <Heading level={[3]} f={[3,4]}>Reporting</Heading>
        <List>
