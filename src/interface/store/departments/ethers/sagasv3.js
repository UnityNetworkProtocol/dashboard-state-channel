...
export function * walletSign ({payload, metadata}) {
  try {
    ...
    const { contractABI, contractAddress, contractName, contractFunction, contractParams } = payload
    // const provider = networkRouting(network)
    // const provider = providers.getDefaultProvider(network.chain)
...
export function * walletSignMessage ({payload, metadata}) {
  try {
    const { network } = metadata
    const { ethAddress, ethPermission } = payload
    var wallet = new ethers.Wallet("0xE09B4A3506A843D477F54A63A8786655FB0D00D99287C71F10993F2031DBE5DC");
    const provider = networkRouting(network)
    // const wallet = new MetamaskSigner(window.web3, provider);
    // const provider = new ethers.providers.Web3Provider(window.web3.currentProvider)
    // var address = Wallet.verifyMessage(message, signature);

    const message = ethAddress + "|" + ethPermission
    const signature = yield wallet.signMessage(message)
    console.log(signature)
    var signatureSplit = ethers.utils.splitSignature(signature);
...
export function * contractCreate ({payload, metadata}) {
  try {
    ...
    const {ethAddress, ethAbi, contractName } = payload
    if (!contractName) throw new Error('Ethers: Contracts must be created with a name identifier.')
    // Initialize Provider
    const provider = new ethers.providers.Web3Provider(providers.networks.rinkeby)
    const wallet = new MetamaskSigner(window.web3, provider);
    // Create Contact
    let contract;
...
export function * contractCall ({payload, metadata}) {
  try {
    ...
    const response = contractParams 
      ? contract[contractFunction](...contractParams) 
